...
def _build_skip_trace_html(
    submitted: int,
    found: int,
    not_found: int,
    activated: int,
) -> str:
    """Build an HTML email summarizing skip-trace results."""
    now = datetime.now(timezone.utc).strftime("%B %d, %Y at %I:%M %p UTC")
    hit_rate = f"{found / submitted * 100:.0f}" if submitted else "0"
    return f"""\
<!DOCTYPE html>
<html>
<head>
      ...
      <td align="center">
        <table width="600" cellpadding="0" cellspacing="0" style="background-color:#ffffff;border-radius:12px;overflow:hidden;box-shadow:0 2px 8px rgba(0,0,0,0.08);">
          ...
          <tr>
            <td style="padding:32px;">
              <p style="margin:0 0 20px;font-size:16px;color:#374151;">
                A skip-trace batch has finished processing. Here are the results:
              </p>
              ...
              <table width="100%" cellpadding="0" cellspacing="0" style="border:1px solid #e5e7eb;border-radius:8px;overflow:hidden;">
                ...
                <tr>
                  ...
                  <td style="padding:12px 16px;font-size:16px;color:#16a34a;font-weight:600;border-bottom:1px solid #e5e7eb;">
                    {found} ({hit_rate}% hit rate)
                  </td>
                </tr>
                <tr style="background-color:#f9fafb;">
                  <td style="padding:12px 16px;font-size:14px;color:#6b7280;font-weight:bold;border-bottom:1px solid #e5e7eb;">
                ...
                <tr>
                  ...
                  <td style="padding:12px 16px;font-size:16px;color:#4338ca;font-weight:600;">
                    {activated} leads ready for outreach
                  </td>
                </tr>
              </table>
              ...
              <div style="margin-top:24px;text-align:center;">
                <p style="margin:0;font-size:14px;color:#6b7280;">
                  Activated leads are now in the outreach queue.
                </p>
              </div>
            </td>
          </tr>
          ...
          <tr>
            <td style="background-color:#f9fafb;padding:16px 32px;text-align:center;border-top:1px solid #e5e7eb;">
              <p style="margin:0;font-size:12px;color:#9ca3af;">
                Solar Command &mdash; Skip-Trace Notification
              </p>
            </td>
          </tr>
        </table>
...
async def send_skip_trace_notification(
    ...
    """Send a skip-trace summary email to the business owner.
    ...
    if not settings.sendgrid_api_key:
        logger.info("SendGrid not configured — skipping skip-trace email notification")
        return False

    if not settings.notification_email:
        logger.info("NOTIFICATION_EMAIL not set — skipping skip-trace email notification")
        return False

    try:
        html_body = _build_skip_trace_html(
            submitted=submitted,
            found=found,
            not_found=not_found,
            activated=activated,
        ...
        message = Mail(
            ...
            subject=f"Skip-Trace Complete — {found}/{submitted} contacts found, {activated} activated",
        ...
        if response.status_code in (200, 201, 202):
            logger.info(
                "Skip-trace notification email sent to %s (%d found, %d activated)",
                settings.notification_email, found, activated,
            )
            return True
        else:
            logger.error("SendGrid returned status %s for skip-trace notification", response.status_code)
            return False

    except Exception:
        logger.exception("Failed to send skip-trace notification email")
        return False
